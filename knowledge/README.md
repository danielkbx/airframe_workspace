...
The knowledge base records what Airframe knows and why. It is not the normative product specification: accepted decisions belong in `../.agents/MEMORY.md`, implemented structure in `../.agents/ARCHITECTURE.md`, current execution in `../.agents/PLAN.md`, unapproved ideas in `../.agents/BACKLOG.md`, and repeatable tool workflows in `../.agents/TOOLING.md`.
...
The specialized [SpeedyBee reverse-engineering lab notebook](SPEEDYBEE_REVERSE_ENGINEERING.md) remains separate; connectivity research links to it instead of duplicating it.

## Maintenance Rules

- Add unsettled observations to `../.agents/RESEARCH.md#research-inbox` first.
