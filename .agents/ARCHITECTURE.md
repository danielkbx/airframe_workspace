...
- Airframe-document log rows read automatic tags from `AirframeWorkspaceController`. Cache hits publish immediately; missing/incompatible/corrupt per-segment caches are rebuilt at utility priority from Reader scan facts and the versioned health report, then silently saved at `indexes/automatic-log-tags/<sourceSHA256>/<segmentIndex>.json`.
