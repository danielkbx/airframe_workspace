...
- iOS: `WindowGroup` always routes through `HomeView`, including UI-test fixture launches, then owns at most one `AirframeUIDocument` workspace. The document toolbar exposes an explicit Home action that completes the existing flush/platform-close lifecycle before returning to the start surface; close failure leaves the document open and uses the existing error presentation.
