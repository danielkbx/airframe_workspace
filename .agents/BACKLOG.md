...
- Craft section follow-ups (base implemented 2026-07-20: collapsible Graph inspector section, pseudo-3D Canvas craft, rotor ring gauges, complementary-filter attitude in `BlackboxAnalysis/Attitude/`):
  ...
  - Non-quad Craft layouts (hex/octo/bi/X8) once a generic motor placement model is worth it; the Craft inspector still gates on exactly 4 motor fields. Generic data views support 2 through 8 motors. LOG00027 is a local 8-motor/X8 example with `motor[0...7]`, `eRPM[0...7]`, `gyroADC[0...2]`, and `accSmooth[0...2]`; only the Craft inspector hides it because the app-side view guard requires exactly 4 motor series, not because attitude inputs are missing.
