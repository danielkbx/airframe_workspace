...
- Improve BLE import feedback and throughput: make sub-percent progress visibly legible, show transferred bytes/rate or equivalent native detail, and evaluate Configurator-compatible Huffman-compressed FlashFS reads after correctness is stable. The first SpeedyBee V2 hardware run transferred uncompressed 512-byte blocks at roughly 3.4 KiB/s.
