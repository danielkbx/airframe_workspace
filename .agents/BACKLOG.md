...
- QuickLook preview and thumbnail extensions for `.bbl`/`.bfl`/`.airframe` (user deferred 2026-08-17 in favor of Spotlight first). The reusable building blocks are the unused `ReaderHeaderPreviewOptions.quickLook` preset plus the `AirframeSpotlight` mapper and `ContainerMetadataSummary`; choose Quick Look's own current Xcode target template rather than copying Spotlight target wiring.
- Retest the classic Spotlight import extension end-to-end on a release macOS. On the 2026-08-17 host, the system's `mdimport` discovers the corrected `Contents/PlugIns` importer but is rejected by BackgroundTaskManagement for lacking Apple's private management entitlement; physical iPad installation and launch are already green. After macOS indexing works, verify the no-duplicate-results contract.
