...
- Per-window transient UI state lives in `DocumentStateStore`. Timeline Range, Current Position, and Graph viewport are routed to the active log store; all other view configuration is document-wide. Package-persistent state lives in metadata.
