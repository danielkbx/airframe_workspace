...
- Hardware validation with `SBADAPTER3_C571` reached Betaflight identity and a full 16 MiB FlashFS through SpeedyBee V2 (`ABF0`, write `ABF1`, notify `ABF2`). BLE uses uncompressed 512-byte dataflash reads; 4,096-byte uncompressed responses produced checksum failures. The 512-byte transfer grew the temporary file steadily at roughly 3.4 KiB/s in the observed run.
