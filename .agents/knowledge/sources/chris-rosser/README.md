...
The PDFs are stored in the private repository through Git LFS. The scoped `.gitattributes` rule applies only to PDFs under `.agents/knowledge/sources/`; the files must never be copied into the public `Airframe/` repository.
