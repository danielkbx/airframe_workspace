...
- Scope: verbatim pre-Knowledge-Base research snapshot retained to prevent information loss during thematic migration
- Normative decisions: none
- Related implementation: see the active thematic Knowledge Base files

This snapshot is not maintained and may contain superseded commit IDs or interpretations. Active claims belong in the themed files listed in `README.md`. The original content follows verbatim.
...
- PIDtoolbox v0.23 estimates filter delay in `PTspecUIcontrol.m` on Roll only. It applies equal 50-sample `smooth` operations and MATLAB `finddelay` with an approximately 3 ms maximum lag to `debug[0]` (`GYRO_SCALED`) versus `gyroADC[0]`, then to a scaled `-diff(gyroADC[0])` versus `axisD[0]`; values below 0.1 ms are discarded and Gyro/D-Term/Total are shown. The current Blackbox Explorer reference has no corresponding delay calculation. Modern Betaflight logs `gyroUnfilt[0...2]` independently of debug mode by default (`blackbox_disable_gyrounfilt` can disable it), so it is Airframe's preferred prefilter source.
...
- Uncompressed 4,096-byte FlashFS responses were not reliable through this BLE adapter and produced MSP checksum mismatches. Uncompressed 512-byte reads transferred steadily without checksum errors.
- Betaflight compressed `MSP_DATAFLASH_READ` responses contain the address, a data-size field, compression type `1`, a little-endian uncompressed character count, and the fixed-tree Huffman bitstream. The firmware only reports complete 256-byte source chunks. If the compressed output budget cannot fit one complete source chunk, a valid response can report zero decoded characters.
- On the hardware-validated SpeedyBee V2 path, a 256-byte compressed budget produced a zero-character response, while 512-byte compressed responses eventually failed. A 400-byte budget is large enough for a worst-case 256-byte source chunk encoded with 12-bit codes and remained stable for more than four minutes: 1,577 responses and 837,120 decoded bytes without retries, timeouts, checksum failures, or disconnection. Observed throughput remained roughly 3.2 KiB/s, so compression correctness alone does not resolve BLE performance.
- `Logs/Faulty/LOG00004.bfl` is an 8 MiB FlashFS dump whose real terminal log-end event is the byte sequence `E FF "End of log" 00` ending at byte offset 744986. Before the fix, Airframe scanned the whole 8 MiB file and reported 438.463 s. After scanner-level segment truncation it opens byte range `0..<744986` and reports 1.261 s. Validation still reports pre-end frame-jump issues, but issue truncation now occurs at offset 744975 instead of the FlashFS tail near 8 MiB.
- `LOG00004.bfl` also exposed a GraphMode startup crash in the always-visible log timeline, not in the graph canvas. The scan overview can contain damaged/inverted main-frame gap endpoints after corruption just before `LogEnd`; app-side timeline projection must normalize and clamp `ReaderMainFrameGap` endpoints before creating `ClosedRange`. The verified failing stack was `DocumentHomeView.LogTimeline.makeModel` at `LogTimelineModel.swift:125`; after normalizing gaps, the macOS app opens the log and switches to GraphMode without a new crash report.
