...
The builder verifies source SHA-256 values and publication approval before copying validated log and configuration blobs. It reads stable, ignored snapshots from `fixtures/logs/Screenshot Sources/`; only the resulting central document is versioned. `Baseline Screenshot Source.airframe` is the last approved central fixture before the notch-overlay addition, while `Notch Overlay Source.airframe` contributes the approved BF 2026.6.1 filter example. Refresh either local snapshot and update its hash only after reviewing the replacement source and its visible privacy-sensitive data.

The document fixes the aircraft presentation to a 5-inch freestyle propeller size and classic Quad X layout. `Long Range Test` overrides the document default with the 6–7-inch Long Range profile. Its visible flights are `Baseline Flight`, `Filter Tune`, `Notch Filter Tune`, `PID Tune`, `Long Range Test`, `CHIRP Tune`, and `Motor Setup Check`.
